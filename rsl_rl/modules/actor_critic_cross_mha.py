        ...
        **kwargs: dict[str, Any],
        ...
        for obs_group in obs_groups["policy"]:
            if len(obs[obs_group].shape) == 4:  # B, C, H, W
                ...
                actor_in_channels_2d.append(obs[obs_group].shape[1]-2)
        ...
        for obs_group in obs_groups["critic"]:
            if len(obs[obs_group].shape) == 4:  # B, C, H, W
                ...
                critic_in_channels_2d.append(obs[obs_group].shape[1]-2)
            elif len(obs[obs_group].shape) == 2:  # B, C
        ...
        # Actor Linear
        self.actor_linear = torch.nn.Linear(
            in_features=num_actor_obs_1d,
            out_features=encoding_dim, #d
            bias=True,
        )

        # Actor Cross MHA
        self.actor_cross = torch.nn.MultiheadAttention(
            embed_dim=encoding_dim,
            num_heads=16, #TODO: para from actor_cross_cfg
            dropout=0.0,
            bias=True,
            add_bias_kv=False,
            add_zero_attn=False,
        ...
        if self.actor_obs_groups_2d:
            ...
            assert critic_cnn_cfg is not None, "An critic CNN configuration is required for 2D actor observations."
            # If a single configuration dictionary is provided, create a dictionary for each 2D observation group
        ...
        # Critic Linear
        self.critic_linear = torch.nn.Linear(
            in_features=num_actor_obs_1d,
            out_features=encoding_dim, #d
            bias=True,
        )
        ...
        # Critic Cross MHA
        self.critic_cross = torch.nn.MultiheadAttention(
            embed_dim=encoding_dim,
            num_heads=16,  # TODO: para from critic_cross_cfg
            dropout=0.0,
            bias=True,
            add_bias_kv=False,
            add_zero_attn=False,
